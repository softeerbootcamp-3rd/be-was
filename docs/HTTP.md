...
- 보안상의 이유로 PUT 및 DELETE는 대부분의 프로덕션 서버에서 지원되지 않는다.
- RESTful API 디자인에서는 리소스의 수정은 PUT, 삭제는 DELETE를 사용하는 것이 규칙적이지만, 특정 상황에서는 POST 메서드를 사용하는 것이 더 편리하거나 안전할 수 있다.
    - PUT 및 DELETE 메서드의 지원 여부는 보안 및 웹 애플리케이션의 설계 및 구현에 따라 다를 수 있음
