...
- Expires 또는 Max-Age를 지정하지 않으면, Session 쿠키로 간주하여 브라우저 종료시 쿠키가 소멸

#### 2) 존재 이유
1. HTTP의 Connectionless
   - HTTP는 TCP 연결을 맺고 요청을 보내면 서버는 응답을 보내고 연결이 끊어짐
2. HTTP의 Stateless
   - HTTP는 상태를 따로 저장하지 않음 
   - 연결이 끊어지는 순간 모든 상태 정보가 사라지게 됨
...
4. expires
   - 쿠키의 만료기간
   - 만료기간이 지나면 쿠키 자동 삭제 (절대적)
   - GMT 형식
5. Max-Age
   - 현재 시간을 기준으로 쿠키를 유지할 기간 지정 (상대적)
6. secure
   - secure 속성을 설정하면 HTTPS 통신이 아닌경우 서버로 쿠키를 전송하지 않음

### 2. Session
- 일정 기간 동안 같은 클라이언트로부터 들어오는 일련의 요구를 하나의 상태로 보고 
그 상태를 일정하게 유지시키는 기술
- 쿠키를 기반으로 하지만 쿠키와 다르게 서버 측에서 저장하고 관리
- 세션 ID를 이용해 클라이언트를 구분

### 3. 쿠키와 세션의 차이
- 쿠키는 클라이언트에 저장 vs. 세션은 서버에 저장
- 세션은 서버에 저장되므로 서버의 자원을 사용
- 서버에 요청을 보내는 사용자가 많을 경우 세션은 부하가 심할 수 있음
- 보안면에서 세션이 더 우수
- 쿠키는 만료시간 동안 파일로 저장되므로 브라우저를 종료해도 정보가 남아있음
