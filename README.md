...
#### 글쓰기 기능 구현
1. index.html 하단에 글쓰기 버튼을 추가한다.
2. 로그인한 사용자가 글쓰기 버튼을 클릭하면 write.html 로 이동한다.
3. 로그인하지 않은 사용자가 글쓰기 버튼을 클릭하면 로그인 페이지로 이동한다.
4. write.html에서는 글의 본문을 입력할 수 있는 폼을 표시한다.
5. 작성한 글의 제목을 index.html에서 보여준다.

#### 세부 글 표시 기능 구현
1. 로그인한 사용자가 글 제목 클릭시 세부 내용을 볼 수 있는 페이지로 이동한다.
2. 로그인하지 않은 사용자가 글 제목 클릭시 로그인 페이지로 이동한다.

#### 프로그래밍 요구사항
1. 404 및 기타 에러 처리 페이지 구현
2. DB 연동(H2와 JDBC를 사용하여 회원정보와 게시글을 저장하도록 구현)

### ✔ 구현 방법
>⚪ 글쓰기 클릭 시 로그인 여부에 따라 redirection

>⚪ 글쓰기 post 시 WritingController에서 h2연결하고 article 정보 저장하고 db에 저장

>⚪ DB에 Article테이블 전체 내용 가져와서 index.html에 동적으로 출력

>⚪ 특정글 선택 시, 로그인이 된 삳태라면 articleId를 URL로 넘겨주고 해당 articleId에 해당되는 행을 가져와서 출력 